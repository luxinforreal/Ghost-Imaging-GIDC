...
Fei Wang et al. 'Far-field super-resolution ghost imaging with a deep neural network constraint'. Light Sci Appl 11, 1 (2022).
...
for i in range(num_patterns):    
    pattern = data['patterns'][:,:,i]   # 找到.mat中的散斑信息[:,:,i]表示的是1200次捕获的散斑图像
    count = count + 1                   # 计数器 后面用来作为归一化的系数
    B_r = data['measurements'][i]       # 桶探测信号，单次捕获得到光强像素和
...
# Build the DNN structure
# (the physical model was embedded in the DNN) y = Ax, y:measurements(known) A:physical model(known) x:object(unknown)
